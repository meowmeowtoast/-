...
const AVAILABLE_COLUMNS: ColumnDef[] = [
  ...
  { id: 'landingPageViews', label: '頁面瀏覽', type: 'number' }, 
  
  // Video
  { id: 'videoViews', label: '影片觀看(3秒)', type: 'number' }, 
...
const DEMO_TABLE_COLS: ColumnDef[] = [
  { id: 'impressions', label: '曝光', type: 'number' },
  { id: 'clicks', label: '點擊', type: 'number' },
  { id: 'ctr', label: 'CTR', type: 'percent' },
  { id: 'cpc', label: 'CPC', type: 'currency' },
  { id: 'linkClicks', label: '連結點擊', type: 'number' },
  { id: 'linkCtr', label: '連結 CTR', type: 'percent' },
  { id: 'linkCpc', label: '連結 CPC', type: 'currency' },
  { id: 'websitePurchases', label: '購買', type: 'number' },
  { id: 'cpa', label: 'CPA', type: 'currency' },
  { id: 'conversionRate', label: 'CVR', type: 'percent' },
  { id: 'spend', label: '花費', type: 'currency' },
];
...
const OVERRIDE_OPTIONS = [
  ...
  { value: 'purchase', label: '網站購買 (Purchases)' },
  { value: 'omni_landing_page_view', label: '頁面瀏覽 (Landing Page Views)' },
  { value: 'video_thruplay_watched_actions', label: 'ThruPlay (Video)' },
  { value: 'lead', label: '潛在客戶 (Leads)' },
  { value: 'onsite_conversion.messaging_conversation_started_7d', label: '開始訊息對話 (Msg Started)' },
];
...
const App: React.FC = () => {
  ...
  // Set default token
  const [selectedTokenId, setSelectedTokenId] = useState("dev-auto-token");
  ...
  const applyPreset = (presetId: string) => {
    ...
    if (def) {
      ...
      setVisibleColumns(def.columns);
      return;
    }
    // Check custom presets
    const cust = customPresets.find(p => p.id === presetId);
    if (cust) {
      setActivePresetId(presetId);
      setVisibleColumns(cust.columns);
    }
  };
  ...
  const saveCurrentAsPreset = (name: string) => {
      ...
      addToast(`已儲存組合: ${name}`, 'success');
  };

  const getTabLabel = (tab: string) => {
      switch(tab) {
          case 'campaign': return '廣告活動 (Campaign)';
          case 'adset': return '廣告受眾 (AdSet)';
          case 'creative': return '廣告素材 (Ad)';
          case 'age': return '年齡分佈';
          case 'gender': return '性別分佈';
          case 'yangyu': return '秧語預設';
          default: return tab;
      }
  };

  useEffect(() => { localStorage.setItem('adflux_projects', JSON.stringify(projects)); }, [projects]);
  useEffect(() => { localStorage.setItem('adflux_tokens', JSON.stringify(storedTokens)); }, [storedTokens]);
  useEffect(() => {
    const storedPresets = localStorage.getItem('adflux_presets');
    if (storedPresets) setCustomPresets(JSON.parse(storedPresets));
  }, []);

  useEffect(() => { setOverrideResultType(""); }, [selectedDebugRow]);
  ...
  useEffect(() => {
      ...
      if (activeTab === 'yangyu') applyPreset('yangyu_default');
      setSortConfig(null);
  }, [activeTab]);
  ...
  useEffect(() => {
      ...
      if (isDateChanged && activeProject?.metaConfig) handleSyncMeta(true);
  ...
  const demographicData = useMemo(() => {
    ...
    const totalRow = calculateTotalRow(result, "總計");
  ...
  const tableTotals = useMemo(() => {
    ...
    return calculateTotalRow(filteredData, "總計");
  }, [filteredData]);
  ...
  const handleConnectMeta = async () => {
      ...
      setLoading(true);
      try {
          const rows = await fetchMetaAdsData(tokenToUse, selectedMetaAccount, dateRange.start, dateRange.end, account.currency || 'USD');
          const newProject: Project = {
              id: crypto.randomUUID(),
              name: `Meta: ${account.name}`,
              data: rows,
              currency: account.currency || 'USD',
              metaConfig: { accountId: account.account_id, accountName: account.name, token: tokenToUse, currency: account.currency || 'USD' },
              createdAt: Date.now(),
              updatedAt: Date.now()
          };
  ...
  const handleSyncMeta = async (force: boolean = false) => {
      ...
      setLoading(true);
      try {
          const rows = await fetchMetaAdsData(activeProject.metaConfig.token, activeProject.metaConfig.accountId, dateRange.start, dateRange.end, activeProject.metaConfig.currency || 'USD');
  ...
  const updateProjectName = (id: string, name: string) => { setProjects(prev => prev.map(p => p.id === id ? { ...p, name, updatedAt: Date.now() } : p)); setEditingProjectId(null); };
  ...
  const handleUpload = async (files: File[]) => { 
    ...
    try {
      ...
      setProjects(prev => prev.map(p => {
        if (p.id === activeProjectId) return { ...p, data: [...p.data, ...newRows], currency: detectedCurrency, updatedAt: Date.now() };
        return p;
      }));
  ...
  const saveToken = () => { 
      ...
      const newToken: StoredToken = { id: crypto.randomUUID(), alias: newTokenAlias, token: newTokenValue, createdAt: Date.now() };
      setStoredTokens(prev => [...prev, newToken]);
      setNewTokenAlias("");
      setNewTokenValue("");
      addToast("Token 已儲存", "success");
  };
  const deleteToken = (id: string) => { setStoredTokens(prev => prev.filter(t => t.id !== id)); if (selectedTokenId === id) setSelectedTokenId(""); };
  ...
  // Override handler (Shortened for brevity but logically same)
  const applyOverride = () => {
          ...
          const newData = p.data.map(row => {
              if (row.id === selectedDebugRow.id) {
                  ...
                  if (newVal === 0) {
                      ...
                      else if (targetType === 'video_thruplay_watched_actions') newVal = row.videoViews; 
                  }
                  let newCost = 0;
                  const cpaList = row.costPerActionType as any[]; 
                  ...
                  if (newCost === 0 && newVal > 0) newCost = row.spend / newVal;
                  const labelOpt = OVERRIDE_OPTIONS.find(o => o.value === targetType);
                  const newLabel = labelOpt ? labelOpt.label.split(' (')[0] : targetType;
                  const updatedRow = { ...row, conversions: newVal, resultType: newLabel, costPerResult: newCost, optimizationGoal: `MANUAL_${targetType.toUpperCase()}` };
  ...
  const handleAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const result = await generateInsights(filteredData.length > 0 ? filteredData : activeProject.data);
      setAnalysisResult(result);
    } catch (e) {
      addToast("AI 分析服務暫時無法使用", "error");
    } finally {
  ...
  return (
    <div className="flex h-screen bg-[#09090b] text-zinc-300 font-sans overflow-hidden selection:bg-indigo-500/30">
      ...
      <Dialog isOpen={isExportDialogOpen} onClose={() => setIsExportDialogOpen(false)} title="下載報表設定">
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                  {EXPORT_TYPES.map(type => (
                      <label key={type.id} className="flex items-center gap-3 p-3 rounded-lg border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 cursor-pointer transition-colors">
                          <Checkbox checked={selectedExportTypes.includes(type.id)} onChange={(e) => { if(e.target.checked) setSelectedExportTypes([...selectedExportTypes, type.id]); else setSelectedExportTypes(selectedExportTypes.filter(id => id !== type.id)); }} />
              ...
              <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button onClick={executeExport} disabled={selectedExportTypes.length === 0} className="bg-[#1877F2] hover:bg-[#166fe5] text-white">確認下載</Button>
              </div>
          </div>
      </Dialog>
         ...
         <div className="space-y-4 max-h-[70vh] overflow-y-auto">
             ...
             <div className="bg-zinc-900/50 border border-zinc-800/50 p-4 rounded-lg">
                 <h4 className="text-sm font-medium text-zinc-200 mb-3 flex items-center gap-2">
                     <Edit2 size={14} className="text-indigo-400"/> 手動替換成果類型 (Override Result)
                 </h4>
                 <div className="flex gap-2">
                     <Select value={overrideResultType} onChange={e => setOverrideResultType(e.target.value)} className="flex-1">
                        <option value="">選擇要替換的指標...</option>
                        {OVERRIDE_OPTIONS.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                     </Select>
                     <Button onClick={applyOverride} disabled={!overrideResultType}>套用變更</Button>
                 </div>
                 <p className="text-[10px] text-zinc-500 mt-2">注意：此操作會強制將該列的「成果」與「每次成果成本」改為您選擇的指標。</p>
             </div>
             <div>
                 ...
                 {selectedDebugRow?.rawActions && selectedDebugRow.rawActions.length > 0 ? (
                     <pre className="bg-zinc-950 p-3 rounded-md border border-zinc-800 text-[10px] text-zinc-300 font-mono overflow-auto max-h-[400px] whitespace-pre-wrap">{JSON.stringify(selectedDebugRow.rawActions, null, 2)}</pre>
      ...
      <Dialog isOpen={!!projectToDelete} onClose={() => setProjectToDelete(null)} title="確認刪除專案">
          <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-red-950/30 border border-red-900/50 rounded-md text-red-300"><AlertTriangle size={20} className="shrink-0" /><p className="text-sm">確定要刪除此專案嗎？此動作無法復原，所有數據將會遺失。</p></div>
              <div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setProjectToDelete(null)}>取消</Button><Button variant="danger" onClick={confirmDeleteProject}>確認刪除</Button></div>
          </div>
      </Dialog>
            ...
            {!showExportHelp ? (
                <>
                    ...
                    <div className="flex items-center justify-between px-2 pt-2">
                         ...
                         <Button variant="ghost" size="sm" className="text-xs gap-1 h-6 text-indigo-400 hover:text-indigo-300" onClick={() => setShowExportHelp(true)}><HelpCircle size={12} />如何匯出/查看素材影像？</Button>
                    </div>
                </>
            ) : (
                <div className="bg-zinc-900/50 p-4 rounded-lg space-y-3">
                    <div className="flex items-center justify-between border-b border-zinc-800 pb-2"><h4 className="text-sm font-medium text-zinc-100">匯出素材與影像設定</h4><button onClick={() => setShowExportHelp(false)} className="text-xs text-zinc-500 hover:text-zinc-300">返回</button></div>
              ...
              <div className="space-y-2">
                  ...
                  {storedTokens.map(token => (
                      <div key={token.id} className="flex items-center justify-between p-3 rounded-md bg-zinc-900 border border-zinc-800">
                          <div><div className="text-sm font-medium text-zinc-200">{token.alias}</div><div className="text-[10px] text-zinc-500 font-mono">...{token.token.slice(-8)}</div></div>
      ...
      <Dialog isOpen={isMetaModalOpen} onClose={() => setIsMetaModalOpen(false)} title="連結 Meta 廣告帳號">
         <div className="space-y-6">
             <div className="space-y-3">
                 ...
                 {!selectedTokenId && (<Input type="password" placeholder="EAA..." value={metaToken} onChange={(e) => setMetaToken(e.target.value)} className="font-mono text-xs" />)}
                 <Button onClick={handleFetchAccounts} disabled={loading} className="w-full mt-2">{loading ? <RefreshCw className="animate-spin" size={14}/> : "取得帳號列表"}</Button>
             </div>
             {metaAccounts.length > 0 && (
                <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                    <Label>2. 選擇廣告帳號</Label>
                    <Select value={selectedMetaAccount} onChange={(e) => setSelectedMetaAccount(e.target.value)}>{metaAccounts.map(account => (<option key={account.id} value={account.account_id}>{account.name} (ID: {account.account_id})</option>))}</Select>
                </div>
             )}
             <div className="pt-2 flex justify-end gap-2"><Button variant="ghost" onClick={() => setIsMetaModalOpen(false)}>取消</Button><Button disabled={!selectedMetaAccount || loading} onClick={handleConnectMeta} className="bg-[#1877F2] hover:bg-[#166fe5] text-white">{loading ? "連結中..." : "確認並匯入數據"}</Button></div>
         </div>
      </Dialog>

      {/* Sidebar, Header, Main Content Layout ... */}
      {/* ... (Preserving structure but omitting identical lines for brevity in output if allowed, but here is full structure) */}
      <aside style={{ width: isSidebarOpen ? sidebarWidth : 0 }} className={cn("flex-shrink-0 border-r border-zinc-800 bg-[#09090b] flex flex-col transition-all duration-300 ease-in-out relative group/sidebar", !isSidebarOpen && "w-0 border-r-0 overflow-hidden")}>
        <div className="h-14 flex items-center justify-between px-4 border-b border-zinc-800 gap-2 overflow-hidden whitespace-nowrap"><div className="flex items-center gap-2"><div className="h-6 w-6 bg-indigo-500/20 rounded flex items-center justify-center border border-indigo-500/30 flex-shrink-0"><Layers className="text-indigo-400" size={14} /></div><span className="font-semibold text-zinc-100 tracking-tight text-sm">{APP_TITLE}</span></div><button onClick={() => setIsSidebarOpen(false)} className="p-1.5 hover:bg-zinc-800 rounded text-zinc-500 hover:text-zinc-300 transition-colors"><PanelLeftClose size={16} /></button></div>
        <div className="p-3 space-y-2 border-b border-zinc-800"><Button variant="secondary" className="w-full justify-start text-xs h-8 gap-2 bg-zinc-900 hover:bg-zinc-800" onClick={createProject}><Plus size={14} /><span>空白專案 (CSV)</span></Button><Button variant="secondary" className="w-full justify-start text-xs h-8 gap-2 bg-[#1877F2]/10 hover:bg-[#1877F2]/20 text-[#1877F2] border-[#1877F2]/30" onClick={() => setIsMetaModalOpen(true)}><Facebook size={14} /><span>連結 Meta 帳號</span></Button></div>
        <div className="flex-1 overflow-y-auto py-2 px-3 space-y-1 overflow-x-hidden"><div className="flex items-center justify-between px-2 mb-2"><span className="text-xs font-medium text-zinc-500 uppercase tracking-wider whitespace-nowrap">專案列表</span></div>{projects.map(project => (<div key={project.id} className={cn("group flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-colors cursor-pointer relative", activeProjectId === project.id ? "bg-zinc-800/80 text-zinc-100" : "text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200")} onClick={() => setActiveProjectId(project.id)}>{project.metaConfig ? <Facebook size={14} className={cn("flex-shrink-0", activeProjectId === project.id ? "text-[#1877F2]" : "text-zinc-600")} /> : <Folder size={14} className={cn("flex-shrink-0", activeProjectId === project.id ? "text-indigo-400" : "text-zinc-600")} />}{editingProjectId === project.id ? (<input autoFocus className="bg-zinc-900 border border-zinc-700 rounded px-1 py-0.5 w-full text-xs text-zinc-100" value={newProjectName} onChange={e => setNewProjectName(e.target.value)} onBlur={() => updateProjectName(project.id, newProjectName)} onKeyDown={e => e.key === 'Enter' && updateProjectName(project.id, newProjectName)} onClick={e => e.stopPropagation()} />) : <span className="truncate flex-1">{project.name}</span>}<div className={cn("hidden group-hover:flex items-center gap-1 absolute right-2 bg-zinc-800/80 rounded pl-1", activeProjectId === project.id && "bg-transparent")}><button onClick={(e) => { e.stopPropagation(); setEditingProjectId(project.id); setNewProjectName(project.name); }} className="p-1 hover:text-indigo-400"><Edit2 size={12} /></button><button onClick={(e) => { e.stopPropagation(); initiateDeleteProject(project.id); }} className="p-1 hover:text-red-400"><Trash2 size={12} /></button></div></div>))}</div>
        <div className="p-3 border-t border-zinc-800 flex flex-col gap-2"><Button variant="ghost" onClick={() => setIsTokenManagerOpen(true)} className="w-full justify-start text-xs h-8 gap-2 text-zinc-500"><Key size={14} /><span>Token 管理</span></Button><div className="flex items-center justify-between text-xs text-zinc-600 px-1 pt-2 border-t border-zinc-800/50"><div className="flex gap-2"><span>v1.9.1</span><span>Pro</span></div></div></div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        {!isSidebarOpen && (<div className="absolute top-3 left-4 z-50 animate-in fade-in zoom-in duration-200"><button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-zinc-800 border border-zinc-700 rounded-md text-zinc-400 hover:text-zinc-100 shadow-lg hover:shadow-indigo-500/10 transition-all"><Layers size={18} /></button></div>)}
        {activeProject ? (
          <>
            <header className="h-14 flex items-center justify-between px-6 border-b border-zinc-800 bg-[#09090b]/50 backdrop-blur-sm z-10 transition-all duration-300" style={{ paddingLeft: !isSidebarOpen ? '60px' : '24px' }}>
               <div className="flex items-center gap-4"><div><div className="flex items-center gap-2">{activeProject.metaConfig && <Facebook size={12} className="text-[#1877F2]" />}<h2 className="text-sm font-medium text-zinc-100">{activeProject.name}</h2></div><p className="text-[10px] text-zinc-500">最後更新: {new Date(activeProject.updatedAt).toLocaleDateString()} {new Date(activeProject.updatedAt).toLocaleTimeString()}</p></div></div>
               <div className="flex items-center gap-3">{activeProject.data.length > 0 && (<><Button onClick={handleAnalysis} variant="secondary" className="h-8 text-xs gap-2">{isAnalyzing ? <RefreshCw className="animate-spin" size={14} /> : <Sparkles size={14} className="text-amber-400"/>} AI 分析</Button><Button onClick={handleExportClick} variant="primary" className="h-8 text-xs gap-2"><Download size={14} /> 下載報表</Button></>)}</div>
            </header>
            <main className="flex-1 overflow-y-auto p-6 space-y-6">
              {activeProject.data.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center pb-20">{/* Empty State UI */}
                   {activeProject.metaConfig ? (
                       <div className="text-center space-y-4"><div className="w-16 h-16 bg-[#1877F2]/10 rounded-2xl flex items-center justify-center mx-auto border border-[#1877F2]/20"><Facebook size={32} className="text-[#1877F2]" /></div><h3 className="text-lg font-medium text-zinc-200">Meta API 已連結</h3><p className="text-sm text-zinc-500">點擊上方工具列的「同步」按鈕來抓取數據</p></div>
                   ) : (
                        <div onClick={() => setIsUploadModalOpen(true)} className="border-2 border-dashed border-zinc-800 hover:border-zinc-700 bg-zinc-900/50 rounded-xl p-10 text-center transition-all cursor-pointer"><Upload className="mx-auto h-10 w-10 text-zinc-500 mb-4" /><h3 className="text-lg font-medium text-zinc-200">開始匯入資料</h3><p className="text-sm text-zinc-500 mt-2">點擊開啟上傳視窗</p></div>
                   )}
                </div>
              ) : (
                <>
                  {analysisResult && (<Card className="p-5 border-indigo-500/20 bg-indigo-900/10 animate-in fade-in slide-in-from-top-2"><div className="flex items-start gap-4"><div className="p-2 bg-indigo-500/20 rounded-md shrink-0"><Sparkles className="text-indigo-400" size={18} /></div><div className="space-y-1 flex-1"><h3 className="text-sm font-medium text-indigo-100">AI 成效分析報告</h3><div className="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap font-light opacity-90">{analysisResult}</div></div><button onClick={() => setAnalysisResult(null)} className="text-zinc-500 hover:text-zinc-300"><X size={16} /></button></div></Card>)}
                  <div className="sticky top-0 bg-[#09090b] py-2 z-50 border-b border-zinc-800/0">
                    <div className="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between">
                        <div className="w-full xl:w-auto overflow-x-auto no-scrollbar pb-1 xl:pb-0"><div className="flex p-1 bg-zinc-900 rounded-lg border border-zinc-800 shrink-0">{(['campaign', 'adset', 'creative', 'age', 'gender', 'yangyu'] as const).map((tab) => (<button key={tab} onClick={() => { setActiveTab(tab); setSearchQuery(""); }} className={cn("px-3 py-1.5 text-xs font-medium rounded-md transition-all whitespace-nowrap", activeTab === tab ? "bg-zinc-800 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-300")}>{getTabLabel(tab)}</button>))}</div></div>
                        <div className="flex items-center gap-3 flex-wrap w-full xl:w-auto">
                            <div className="relative group shrink-0"><div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-lg px-2 h-9"><Filter size={14} className="text-zinc-500" /><select className="bg-transparent text-xs text-zinc-300 focus:outline-none appearance-none pr-4 cursor-pointer" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)}><option value="all">所有廣告</option><option value="active">刊登中的廣告</option><option value="delivered">已投遞 (有曝光)</option></select></div></div>
                            <div className="relative group shrink-0 w-48"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-indigo-400 transition-colors" size={14} /><input type="text" placeholder="搜尋名稱..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg pl-8 pr-3 py-1.5 text-xs text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-600 focus:ring-1 focus:ring-zinc-600 transition-all" />{searchQuery && <button onClick={() => setSearchQuery("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-zinc-400"><X size={12} /></button>}</div>
                            {activeProject.metaConfig ? (<div className="flex items-center gap-2 shrink-0 relative z-30"><DateRangePicker value={dateRange} onChange={setDateRange} /><button onClick={() => handleSyncMeta(false)} disabled={loading} className="h-9 px-3 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 rounded-lg text-xs text-[#1877F2] hover:text-white font-medium flex items-center gap-1 disabled:opacity-50 transition-colors">{loading ? <RefreshCw size={14} className="animate-spin"/> : <RefreshCw size={14}/>} 同步</button></div>) : (<div className="h-8 shrink-0"><button onClick={() => setIsUploadModalOpen(true)} className="h-full px-3 flex items-center gap-2 bg-zinc-900 border border-dashed border-zinc-700 hover:border-zinc-500 rounded-md cursor-pointer text-xs text-zinc-400 hover:text-zinc-200 transition-colors"><Plus size={14} /><span>新增檔案</span></button></div>)}
                            {(activeTab !== 'age' && activeTab !== 'gender') && (<button onClick={() => setIsColumnModalOpen(!isColumnModalOpen)} className="h-8 w-8 flex items-center justify-center rounded-lg border border-dashed border-zinc-700 text-zinc-500 hover:text-zinc-300 hover:border-zinc-500 transition-colors" title="欄位設定"><Settings size={14} /></button>)}
                        </div>
                    </div>
                  </div>
                  {isColumnModalOpen && activeTab !== 'age' && activeTab !== 'gender' && (<Card className="p-4 animate-in fade-in slide-in-from-top-2 duration-200 mb-4"><div className="flex items-center justify-between mb-4"><h3 className="text-sm font-medium">自訂欄位顯示</h3><div className="flex gap-2"><Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => { const name = prompt("請輸入此檢視模式的名稱:"); if (name) saveCurrentAsPreset(name); }}>儲存組合</Button><button onClick={() => setIsColumnModalOpen(false)} className="text-zinc-500 hover:text-zinc-300"><X size={16}/></button></div></div><div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">{AVAILABLE_COLUMNS.map(col => (<label key={col.id} className="flex items-center gap-2 text-sm text-zinc-400 hover:text-zinc-200 cursor-pointer select-none"><Checkbox checked={visibleColumns.includes(col.id)} onChange={(e) => { if (e.target.checked) setVisibleColumns([...visibleColumns, col.id]); else setVisibleColumns(visibleColumns.filter(c => c !== col.id)); }} />{col.label}</label>))}</div></Card>)}
                  <div className="rounded-xl border border-zinc-800 overflow-hidden bg-zinc-900/30">
                    <div className="overflow-x-auto">
                      {demographicData ? (
                        <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
                            <thead><tr className="border-b border-zinc-800 bg-zinc-900/80"><th className="px-4 py-3 font-medium text-zinc-500 text-xs uppercase tracking-wider">{activeTab === 'age' ? '年齡' : '性別'}</th>{DEMO_TABLE_COLS.map(col => (<th key={col.id} className="px-4 py-3 font-medium text-zinc-500 text-xs uppercase tracking-wider text-right">{col.label}</th>))}</tr></thead>
                            <tbody className="divide-y divide-zinc-800/50">
                                <tr className="bg-amber-900/30 text-amber-100 font-semibold border-b border-zinc-800"><td className="px-4 py-3">{demographicData.total.name}</td>{DEMO_TABLE_COLS.map(col => (<td key={col.id} className="px-4 py-3 tabular-nums text-right">{formatVal(demographicData.total[col.id], col.type, activeProject.currency)}</td>))}</tr>
                                {demographicData.rows.map(row => (<tr key={row.name} className="hover:bg-zinc-800/30 transition-colors"><td className="px-4 py-2.5 font-medium text-zinc-300">{row.name}</td>{DEMO_TABLE_COLS.map(col => (<td key={col.id} className={cn("px-4 py-2.5 text-zinc-400 tabular-nums text-right", col.type === 'currency' && "text-zinc-300")}>{formatVal(row[col.id], col.type, activeProject.currency)}</td>))}</tr>))}
                            </tbody>
                        </table>
                      ) : (
                        <table className="w-full text-left text-sm whitespace-nowrap border-collapse relative">
                            ...
                            {filteredData.slice(0, 200).map((row, index) => {
                                ...
                                return (
                                    ...
                                    {sortedVisibleColumns.map(colId => {
                                        ...
                                        if (colId === 'budget') return <td key={colId} style={stickyStyle} className={cn("px-4 py-2.5 text-zinc-400 tabular-nums text-right", stickyClass)}>{(val === 0 && row.budgetType === 'ABO') ? (<span className="text-xs text-zinc-500">使用廣告組合預算</span>) : (val ? `${formatVal(val, 'currency', activeProject.currency)}${row.budgetType === 'Daily' ? '/日' : '/總'}` : '-')}</td>;
                                        if (colId === 'conversions') return (<td key={colId} className="px-4 py-2.5 text-right cursor-pointer hover:bg-zinc-800/50 transition-colors rounded-sm" onClick={() => setSelectedDebugRow(row)} title="點擊檢視原始數據 (Debug)"><div className="flex flex-col items-end"><span className="text-zinc-200 font-medium border-b border-dotted border-zinc-600 mb-0.5 pb-0.5 leading-none">{val > 0 ? val.toLocaleString() : '-'}</span><span className="text-[10px] text-zinc-500 flex items-center gap-0.5">{row.resultType || '成果'}<Code size={8} className="text-zinc-600"/></span></div></td>);
                                        if (colId === 'costPerResult') return (<td key={colId} className="px-4 py-2.5 text-right"><div className="flex flex-col items-end"><span className="text-zinc-200 font-medium leading-none">{val > 0 ? formatVal(val, 'currency', activeProject.currency) : '-'}</span><span className="text-[10px] text-zinc-500">每次{row.resultType || '成果'}成本</span></div></td>);
                                        if (colId === 'newMessagingConnections') return (<td key={colId} className="px-4 py-2.5 text-zinc-300 text-right tabular-nums">{val > 0 ? val.toLocaleString() : '-'}</td>);
                                        if (colId === 'costPerNewMessagingConnection') return (<td key={colId} className="px-4 py-2.5 text-zinc-300 text-right tabular-nums">{val > 0 ? formatVal(val, 'currency', activeProject.currency) : '-'}</td>);
                                        
                                        return (<td key={colId} className={cn("px-4 py-2.5 text-zinc-400 tabular-nums", alignClass, def?.type === 'currency' && "text-zinc-300", (colId === 'roas' && row.roas > 3) && "text-emerald-400 font-medium")}>{formatVal(val, def?.type || 'text', activeProject.currency)}</td>);
                                    })}
                                    </tr>
                                );
                            })}
                            ...
                            {tableTotals && (
                                <tfoot className="sticky bottom-0 z-40 bg-[#18181b] font-semibold text-zinc-200 border-t-2 border-zinc-700 shadow-[0_-5px_15px_-5px_rgba(0,0,0,0.5)]"><tr>
                                        {sortedVisibleColumns.map(colId => {
                                            const def = AVAILABLE_COLUMNS.find(c => c.id === colId);
                                            const stickyStyle = getStickyStyle(colId, true);
                                            const alignClass = (def?.type === 'number' || def?.type === 'currency' || def?.type === 'percent') ? 'text-right' : 'text-left';
                                            const cellStyle = { ...stickyStyle, backgroundColor: '#18181b', zIndex: stickyStyle.position === 'sticky' ? 40 : undefined };
                                            let content: React.ReactNode = '-';
                                            if (colId === 'campaignName' || (colId === 'name' && !sortedVisibleColumns.includes('campaignName'))) { content = '總計'; } 
                                            else if (['imageUrl', 'platform', 'status', 'budget', 'name'].includes(colId)) { content = ''; } 
                                            else if (['conversions', 'costPerResult', 'cpa', 'conversionRate', 'costPerPageEngagement'].includes(colId)) { content = '-'; }
                                            else { const val = tableTotals[colId as keyof typeof tableTotals]; if (typeof val === 'number') { content = formatVal(val, def?.type || 'number', activeProject.currency); } }
                                            return (<td key={colId} style={cellStyle} className={cn("px-4 py-3 tabular-nums text-zinc-100", alignClass, cellStyle.position === 'sticky' ? "border-r-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]" : "")}>{content}</td>)
                                        })}
                                </tr></tfoot>
                            )}
                        </table>
                    ...
                    <div className="px-4 py-2 border-t border-zinc-800 bg-zinc-900/50 text-[10px] text-zinc-600 flex justify-between">{demographicData ? (<span>顯示 {demographicData.rows.length} 筆加總資料</span>) : (<span>顯示 {Math.min(filteredData.length, 200)} / {filteredData.length} 筆資料 (僅顯示前200筆)</span>)}{activeProject.data.length > 0 && <span className="opacity-70">資料來源: {activeProject.name}</span>}</div>
                  </div>
                </>
              )}
            </main>
          </>
        ) : (<div className="flex-1 flex flex-col items-center justify-center text-zinc-500"><div className="w-16 h-16 bg-zinc-900 rounded-2xl flex items-center justify-center mb-4 border border-zinc-800"><Folder size={32} className="opacity-20" /></div><h3 className="text-lg font-medium text-zinc-300 mb-2">尚未選擇專案</h3><p className="text-sm max-w-xs text-center mb-6">請從左側選擇一個專案，或是建立新專案來開始管理您的廣告報表。</p><div className="flex gap-3"><Button onClick={createProject}>空白專案</Button><Button variant="secondary" onClick={() => setIsMetaModalOpen(true)}>連結 Meta</Button></div></div>)}
      </div>
    </div>
  );
};
